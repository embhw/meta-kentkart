...
_generate_boot_image() {
	...
	if [ -e ${WORKDIR}/temp_boot ]; then
		rm -rf ${WORKDIR}/temp_boot
		mkdir -p ${WORKDIR}/temp_boot
	else
		mkdir -p ${WORKDIR}/temp_boot
	fi
	...
	if [ -e ${WORKDIR}/boot.img ]; then
		...
		dd if=/dev/zero of=${WORKDIR}/boot.img seek=${BOOT_SPACE} count=1 bs=1024
	else
		dd if=/dev/zero of=${WORKDIR}/boot.img seek=${BOOT_SPACE} count=1 bs=1024
	fi

	cp ${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}-${MACHINE}.bin ${WORKDIR}/temp_boot/${KERNEL_IMAGETYPE}

	# Copy boot scripts
	for item in ${BOOT_SCRIPTS}; do
		src=`echo $item | awk -F':' '{ print $1 }'`
		dst=`echo $item | awk -F':' '{ print $2 }'`
		cp ${DEPLOY_DIR_IMAGE}/$src ${WORKDIR}/temp_boot/$dst
	done
		...
		for DTS_FILE in ${KERNEL_DEVICETREE}; do
			...
			if [ -e "${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}-${DTS_BASE_NAME}.dtb" ]; then
				...
				if [ $kernel_bin = $kernel_bin_for_dtb ]; then
					cp ${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}-${DTS_BASE_NAME}.dtb ${WORKDIR}/temp_boot/${DTS_BASE_NAME}.dtb
				fi
			else
	...
	#TODO: Check extlinux support - M.E.
        ...
        if [ -n "${IMAGE_BOOTFILES}" ]; then
            for IMAGE_FILE in ${IMAGE_BOOTFILES}; do
                if [ -e "${DEPLOY_DIR_IMAGE}/${IMAGE_FILE}" ]; then
                    cp ${DEPLOY_DIR_IMAGE}/${IMAGE_FILE} ${WORKDIR}/temp_boot/${IMAGE_FILE}
                else
    ...
    if ${@bb.utils.contains('COMBINED_FEATURES', 'optee', 'true', 'false', d)}; then
	    ...
	    cp ${DEPLOY_DIR_IMAGE}/${UTEE_FILE} ${WORKDIR}/temp_boot/${UTEE_FILE}
        done
    fi
	mkfs.ext4 -L "BOOT-KK" -F -i 4096 -O ^extent ${WORKDIR}/boot.img -d ${WORKDIR}/temp_boot
	fsck.ext4 -pvfD ${WORKDIR}/boot.img || [ $? -le 3 ]
}
...
generate_imx_sdcard () {
	...
	parted -s ${SDCARD} unit KiB mkpart primary ext4 ${IMAGE_ROOTFS_ALIGNMENT} $(expr ${IMAGE_ROOTFS_ALIGNMENT} \+ ${BOOT_SPACE_ALIGNED})
